...
if __name__ == '__main__':
    ...
    input_node = tf.compat.v1.placeholder(
        tf.float32, shape=(None, h, w, 3), name='image')

    net, pretrain_path, last_layer = get_network(
        args.model, input_node, None, trainable=False)
    if args.quantize:
        g = tf.compat.v1.get_default_graph()
        tf.quantization.quantize.create_eval_graph(input_graph=g)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        flops = tf.compat.v1.profiler.profile(
            None, cmd='graph', options=tf.compat.v1.profiler.ProfileOptionBuilder.float_operation())

...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model', default='mobilenet_v2_1.4', help='model name')
    parser.add_argument('--datapath', type=str,
                        default='/data/public/rw/coco/annotations')
    ...
    with tf.device(tf.DeviceSpec(device_type="CPU")):
        input_node = tf.compat.v1.placeholder(tf.float32, shape=(
            args.batchsize, args.input_height, args.input_width, 3), name='image')
        vectmap_node = tf.compat.v1.placeholder(tf.float32, shape=(
            args.batchsize, output_h, output_w, 38), name='vectmap')
        heatmap_node = tf.compat.v1.placeholder(tf.float32, shape=(
            args.batchsize, output_h, output_w, 19), name='heatmap')

        # prepare data
        df = get_dataflow_batch(args.datapath, True,
                                args.batchsize, img_path=args.imgpath)
    ...
    q_inp_split, q_heat_split, q_vect_split = tf.split(
        q_inp, args.gpus), tf.split(q_heat, args.gpus), tf.split(q_vect, args.gpus)
        ...
        with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_id)):
            with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=(gpu_id > 0)):
                ...
                for idx, (l1, l2) in enumerate(zip(l1s, l2s)):
                    loss_l1 = tf.nn.l2_loss(tf.concat(
                        l1, axis=0) - q_vect_split[gpu_id], name='loss_l1_stage%d_tower%d' % (idx, gpu_id))
                    loss_l2 = tf.nn.l2_loss(tf.concat(
                        l2, axis=0) - q_heat_split[gpu_id], name='loss_l2_stage%d_tower%d' % (idx, gpu_id))
                    losses.append(tf.reduce_mean(
                        input_tensor=[loss_l1, loss_l2]))
    ...
    with tf.device(tf.DeviceSpec(device_type="GPU")):
        ...
        total_loss_ll_paf = tf.reduce_sum(
            input_tensor=last_losses_l1) / args.batchsize
        total_loss_ll_heat = tf.reduce_sum(
            input_tensor=last_losses_l2) / args.batchsize
        total_loss_ll = tf.reduce_sum(
            input_tensor=[total_loss_ll_paf, total_loss_ll_heat])
        ...
        if ',' not in args.lr:
            ...
            learning_rate = tf.compat.v1.train.cosine_decay(
                starter_learning_rate, global_step, args.max_epoch * step_per_epoch, alpha=0.0)
        else:
            lrs = [float(x) for x in args.lr.split(',')]
            boundaries = [step_per_epoch * 5 *
                          i for i, _ in range(len(lrs)) if i > 0]
    ...
    if args.quant_delay >= 0:
        ...
        tf.quantization.quantize.create_training_graph(
            input_graph=g, quant_delay=args.quant_delay)
    ...
    sample_valid = tf.compat.v1.placeholder(
        tf.float32, shape=(12, 640, 640, 3))
    train_img = tf.compat.v1.summary.image('training sample', sample_train, 4)
    valid_img = tf.compat.v1.summary.image(
        'validation sample', sample_valid, 12)
    valid_loss_t = tf.compat.v1.summary.scalar("loss_valid", valid_loss)
    valid_loss_ll_t = tf.compat.v1.summary.scalar(
        "loss_valid_lastlayer", valid_loss_ll)
    merged_validate_op = tf.compat.v1.summary.merge(
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        file_writer = tf.compat.v1.summary.FileWriter(
            os.path.join(logpath, args.tag), sess.graph)
        ...
        while True:
            ...
            if gs_num - last_gs_num >= 500:
                ...
                logger.info('epoch=%.2f step=%d, %0.4f examples/sec lr=%f, loss=%g, loss_ll=%g, loss_ll_paf=%g, loss_ll_heat=%g' % (gs_num /
                                                                                                                                    step_per_epoch, gs_num, batch_per_sec * args.batchsize, lr_val, train_loss, train_loss_ll, train_loss_ll_paf, train_loss_ll_heat))
                last_gs_num = gs_num
            ...
            if gs_num - last_gs_num2 >= 2000:
                # save weights
                saver.save(sess, os.path.join(modelpath, args.tag,
                                              'model_latest'), global_step=global_step)
                ...
                for images_test, heatmaps, vectmaps in validation_cache:
                    lss, lss_ll, lss_ll_paf, lss_ll_heat, vectmap_sample, heatmap_sample = sess.run(
                        ...
                        feed_dict={q_inp: images_test,
                                   q_vect: vectmaps, q_heat: heatmaps}
                    )
                ...
                logger.info('validation(%d) %s loss=%f, loss_ll=%f, loss_ll_paf=%f, loss_ll_heat=%f' % (
                    total_cnt, args.tag, average_loss / total_cnt, average_loss_ll / total_cnt, average_loss_ll_paf / total_cnt, average_loss_ll_heat / total_cnt))
                last_gs_num2 = gs_num

                sample_image = [enqueuer.last_dp[0][i] for i in range(4)]
                outputMat = sess.run(
                    outputs,
                    feed_dict={q_inp: np.array(
                        (sample_image + val_image) * max(1, (args.batchsize // 16)))}
                )
                ...
                for i in range(len(sample_image)):
                    ...
                    test_result = test_result.reshape(
                        [640, 640, 3]).astype(float)
                    sample_results.append(test_result)

                test_results = []
                for i in range(len(val_image)):
                    test_result = CocoPose.display_image(val_image[i], heatMat[len(
                        sample_image) + i], pafMat[len(sample_image) + i], as_numpy=True)
                    test_result = cv2.resize(test_result, (640, 640))
                    test_result = test_result.reshape(
                        [640, 640, 3]).astype(float)
